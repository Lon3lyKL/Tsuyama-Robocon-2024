...
void setup() {
  ...
  firingServo.attach(pwmPinServoFire, 500, 2500);
  ...
  ledcSetup(2, 5000, 8);      // Set up Channel 2

  ledcAttachPin(pwmPinA, 2);  // Attach Motor A's ENA to PWM channel 0
  ledcAttachPin(pwmPinB, 2);  // Attach Motor B's ENB to PWM channel 1
  ledcAttachPin(pwmPinC, 2);  // Attach Motor C's ENA to PWM channel 2
  ledcAttachPin(pwmPinD, 2);  // Attach Motor D's ENB to PWM channel 3
...
void loop() {
  ...
  if (R1) { // If R1 pressed, servo goes to 90
    firingServo.writeMicroseconds(map(90, 0, 300, 500, 2500));
  } else { // If R1 released, servo goes to 0
    firingServo.writeMicroseconds(500);
  }
  ...
  Serial.print(vel_X2);
  Serial.print("\tLift: ");
  Serial.print(liftingServo.read());
  Serial.print("\tFire: ");
  Serial.println(firingServo.read());
}
